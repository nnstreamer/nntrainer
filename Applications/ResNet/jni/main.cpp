 ...
 * @brief	This is Resnet18 Example
 *
 */
 ...
 */
const unsigned int num_class = 10;
...
void ExtractFeatures(std::string p, std::vector<float> &input_img,
  ...
  if (val) {
    ss << std::setw(4) << std::setfill('0') << (5000 - (index % num_val));
  } else {
...
int getBatch_train_file(float **outVec, float **outLabel, bool *last,
  ...
  std::string filename = "resnet_trainingSet.dat";
...
int getBatch_val_file(float **outVec, float **outLabel, bool *last,
  ...
  std::string filename = "resnet_valSet.dat";
  std::ifstream F(filename, std::ios::in | std::ios::binary);
  ...
  for (unsigned int i = val_count; i < val_count + batch_size; i++) {
    ...
    for (unsigned int j = 0; j < feature_size; ++j)
      outVec[0][count * feature_size + j] = o[j] / 255.0;
    for (unsigned int j = 0; j < num_class; ++j)
      outLabel[0][count * num_class + j] = l[j];
    count++;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    std::cout << "./nntrainer_resnet resnet.ini resource\n";
    exit(-1);
  }
