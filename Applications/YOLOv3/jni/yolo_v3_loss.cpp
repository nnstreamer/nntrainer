...
void msePrime(nntrainer::Tensor &pred, nntrainer::Tensor &ground_truth,
  ...
  if (outgoing_derivative.divide_i(divider) != ML_ERROR_NONE) {
    throw std::runtime_error(
      "[YoloV3LossLayer::calcDerivative] Error when calculating loss");
  }
}
...
calc_iou(nntrainer::Tensor &bbox1_x1, nntrainer::Tensor &bbox1_y1,
  ...
  nntrainer::Tensor output = intersection.divide(unions);
  return output;
}
...
void YoloV3LossLayer::finalize(nntrainer::InitLayerContext &context) {
  nntrainer::TensorDim input_dim =
    context.getInputDimensions()[SINGLE_INOUT_IDX];
  const unsigned int batch_size = input_dim.batch();
  const unsigned int class_number =
    std::get<props::ClassNumber>(yolo_v3_loss_props).get();
  const unsigned int grid_height_number =
    std::get<props::GridHeightNumber>(yolo_v3_loss_props).get();
  const unsigned int grid_width_number =
    std::get<props::GridWidthNumber>(yolo_v3_loss_props).get();
  const unsigned int max_object_number =
    std::get<props::MaxObjectNumber>(yolo_v3_loss_props).get();
  const unsigned int scale_of_pyramid =
    std::get<props::Scale>(yolo_v3_loss_props).get();

  if (scale_of_pyramid == 1) {
    anchors_w = new nntrainer::Tensor({1, 1, NUM_ANCHOR, 1}, anchors_w_buf_1);
    anchors_h = new nntrainer::Tensor({1, 1, NUM_ANCHOR, 1}, anchors_h_buf_1);
  } else if (scale_of_pyramid == 2) {
    anchors_w = new nntrainer::Tensor({1, 1, NUM_ANCHOR, 1}, anchors_w_buf_2);
    anchors_h = new nntrainer::Tensor({1, 1, NUM_ANCHOR, 1}, anchors_h_buf_2);
  } else if (scale_of_pyramid == 3) {
    anchors_w = new nntrainer::Tensor({1, 1, NUM_ANCHOR, 1}, anchors_w_buf_3);
    anchors_h = new nntrainer::Tensor({1, 1, NUM_ANCHOR, 1}, anchors_h_buf_3);
  } else {
    throw std::invalid_argument("Invalid scale value of feature pyramid");
  }
...
void YoloV3LossLayer::forwarding(nntrainer::RunLayerContext &context,
                                 bool training) {
  const unsigned int max_object_number =
    std::get<props::MaxObjectNumber>(yolo_v3_loss_props).get();
  ...
  nntrainer::Tensor &bbox_x_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_x_pred]);
  nntrainer::Tensor &bbox_y_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_y_pred]);
  nntrainer::Tensor &bbox_w_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_w_pred]);
  nntrainer::Tensor &bbox_h_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_h_pred]);

  nntrainer::Tensor &confidence_pred =
    context.getTensor(wt_idx[YoloV3LossParams::confidence_pred]);
  nntrainer::Tensor &class_pred =
    context.getTensor(wt_idx[YoloV3LossParams::class_pred]);

  nntrainer::Tensor &bbox_w_pred_anchor =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_w_pred_anchor]);
  nntrainer::Tensor &bbox_h_pred_anchor =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_h_pred_anchor]);
  ...
  nntrainer::Tensor &bbox_x_gt =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_x_gt]);
  nntrainer::Tensor &bbox_y_gt =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_y_gt]);
  nntrainer::Tensor &bbox_w_gt =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_w_gt]);
  nntrainer::Tensor &bbox_h_gt =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_h_gt]);

  nntrainer::Tensor &confidence_gt =
    context.getTensor(wt_idx[YoloV3LossParams::confidence_gt]);
  nntrainer::Tensor &class_gt =
    context.getTensor(wt_idx[YoloV3LossParams::class_gt]);

  nntrainer::Tensor &bbox_class_mask =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_class_mask]);
  nntrainer::Tensor &iou_mask =
    context.getTensor(wt_idx[YoloV3LossParams::iou_mask]);
  ...
  sigmoid.run_fn(class_pred, class_pred);
...
void YoloV3LossLayer::calcDerivative(nntrainer::RunLayerContext &context) {
  nntrainer::Tensor &bbox_x_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_x_pred]);
  nntrainer::Tensor &bbox_x_pred_grad =
    context.getTensorGrad(wt_idx[YoloV3LossParams::bbox_x_pred]);
  nntrainer::Tensor &bbox_y_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_y_pred]);
  nntrainer::Tensor &bbox_y_pred_grad =
    context.getTensorGrad(wt_idx[YoloV3LossParams::bbox_y_pred]);
  nntrainer::Tensor &bbox_w_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_w_pred]);
  nntrainer::Tensor &bbox_w_pred_grad =
    context.getTensorGrad(wt_idx[YoloV3LossParams::bbox_w_pred]);
  nntrainer::Tensor &bbox_h_pred =
    context.getTensor(wt_idx[YoloV3LossParams::bbox_h_pred]);
  nntrainer::Tensor &bbox_h_pred_grad =
    context.getTensorGrad(wt_idx[YoloV3LossParams::bbox_h_pred]);

  nntrainer::Tensor &confidence_pred =
    context.getTensor(wt_idx[YoloV3LossParams::confidence_pred]);
  nntrainer::Tensor &confidence_pred_grad =
    context.getTensorGrad(wt_idx[YoloV3LossParams::confidence_pred]);
  nntrainer::Tensor &class_pred =
    context.getTensor(wt_idx[YoloV3LossParams::class_pred]);
  nntrainer::Tensor &class_pred_grad =
    context.getTensorGrad(wt_idx[YoloV3LossParams::class_pred]);
  ...
  nntrainer::Tensor &bbox1_width =
    context.getTensor(wt_idx[YoloV3LossParams::bbox1_width]);
  nntrainer::Tensor &bbox1_height =
    context.getTensor(wt_idx[YoloV3LossParams::bbox1_height]);
  nntrainer::Tensor &is_xy_min_max =
    context.getTensor(wt_idx[YoloV3LossParams::is_xy_min_max]);
  nntrainer::Tensor &intersection_width =
    context.getTensor(wt_idx[YoloV3LossParams::intersection_width]);
  nntrainer::Tensor &intersection_height =
    context.getTensor(wt_idx[YoloV3LossParams::intersection_height]);
  nntrainer::Tensor &unions =
    context.getTensor(wt_idx[YoloV3LossParams::unions]);
  ...
  sigmoid.run_prime_fn(class_pred, class_pred, class_pred_grad,
                       class_pred_grad);
...
void YoloV3LossLayer::exportTo(nntrainer::Exporter &exporter,
                               const ml::train::ExportMethods &method) const {
  exporter.saveResult(yolo_v3_loss_props, method, this);
}

void YoloV3LossLayer::setProperty(const std::vector<std::string> &values) {
  auto remain_props = loadProperties(values, yolo_v3_loss_props);
  NNTR_THROW_IF(!remain_props.empty(), std::invalid_argument)
    << "[YoloV3LossLayer] Unknown Layer Properties count " +
         std::to_string(values.size());
}

void YoloV3LossLayer::setBatch(nntrainer::RunLayerContext &context,
                               unsigned int batch) {
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_x_pred], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_y_pred], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_w_pred], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_h_pred], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::confidence_pred], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::class_pred], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_w_pred_anchor], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_h_pred_anchor], batch);

  context.updateTensor(wt_idx[YoloV3LossParams::bbox_x_gt], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_y_gt], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_w_gt], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_h_gt], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::confidence_gt], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::class_gt], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox_class_mask], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::iou_mask], batch);

  context.updateTensor(wt_idx[YoloV3LossParams::bbox1_width], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::bbox1_height], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::is_xy_min_max], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::intersection_width], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::intersection_height], batch);
  context.updateTensor(wt_idx[YoloV3LossParams::unions], batch);
}

unsigned int YoloV3LossLayer::find_responsible_anchors(float bbox_ratio) {
...
void YoloV3LossLayer::generate_ground_truth(
...
nntrainer::Layer *create_yolo_v3_loss_layer() {
  auto layer = new YoloV3LossLayer();
  return layer;
}

void destory_yolo_v3_loss_layer(nntrainer::Layer *layer) { delete layer; }
 ...
 */
extern "C" {
nntrainer::LayerPluggable ml_train_layer_pluggable{create_yolo_v3_loss_layer,
                                                   destory_yolo_v3_loss_layer};
}
