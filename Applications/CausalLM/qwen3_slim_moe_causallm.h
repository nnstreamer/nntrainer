// SPDX-License-Identifier: Apache-2.0
/**
 * Copyright (C) 2025 Eunju Yang <ej.yang@samsung.com>
 *
 * @file   qwen3_moe_causallm.h
 * @date   15 July 2025
 * @see    https://github.com/nnstreamer/nntrainer
 * @author Eunju Yang <ej.yang@samsung.com>
 * @bug    No known bugs except for NYI items
 *
 */

#ifndef __QWEN_SLIM_MOE_CAUSAL_LM_H__
#define __QWEN_SLIM_MOE_CAUSAL_LM_H__


#pragma once
#ifdef _WIN32
#define WIN_EXPORT __declspec(dllexport)
#else
#define WIN_EXPORT
#endif

#include <causal_lm.h>
#include <qwen3_causallm.h>

namespace causallm {

/**
 * @brief Qwen3SlimMoECausalLM class
 * @note  This class inherits Qwewn3CaUSALlm
 */
WIN_EXPORT class Qwen3SlimMoECausalLM : public Qwen3CausalLM {

public:
  static constexpr const char *architectures = "Qwen3SlimMoeForCausalLM";

WIN_EXPORT Qwen3SlimMoECausalLM(json &cfg, json &generation_cfg, json &nntr_cfg) :
    Qwen3CausalLM(cfg, generation_cfg, nntr_cfg) {
    setupParameters(cfg, generation_cfg, nntr_cfg);
  }

  WIN_EXPORT virtual ~Qwen3SlimMoECausalLM() = default;

  WIN_EXPORT std::vector<LayerHandle> createMlp(const int layer_id, int dim,
                                     int hidden_dim,
                                     std::string input_name) override;

  WIN_EXPORT void setupParameters(json &cfg, json &generation_cfg,
                       json &nntr_cfg) override;

  WIN_EXPORT void registerCustomLayers() override;

private:
  unsigned int NUM_EXPERTS;
  unsigned int NUM_EXPERTS_PER_TOK;
};
}; // namespace causallm

#endif /* __QWEN_MOE_CAUSAL_LM_H__ */
