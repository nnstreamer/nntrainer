    ...
    for (int expert_idx = 0; expert_idx < static_cast<int>(num_experts);
      ...
      for (int expert_idx : target_idx_vector) {
        if (need_load[expert_idx]) {
          ...
          //   expert_predict_scores[expert_idx] = alpha * 0.0 + (1-alpha) * expert_predict_scores[expert_idx];
          // }
        } else {
          // hit_count += 1;
          ...
          // } else {
          //   expert_predict_scores[expert_idx] = alpha * 1.0 + (1-alpha) * expert_predict_scores[expert_idx];
          // }
          ...
          //   score += h ? 1.0 : 0.0;
          // }
          // expert_predict_scores[expert_idx] = score / expert_histories[expert_idx].size();
