...
void GptOssMoELayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  for (unsigned int i = 0; i < num_experts; ++i) {
    ...
    expert_up_bias_indices.push_back(context.requestWeight(
      ...
      weight_initializer, weight_regularizer, weight_regularizer_constant,
      weight_decay, "expert_up_bias_" + std::to_string(i), false));
    ...
    expert_gate_bias_indices.push_back(context.requestWeight(
      expert_gate_bias_dim, // Same dimensions as gate projection
      weight_initializer, weight_regularizer, weight_regularizer_constant,
      weight_decay, "expert_gate_bias_" + std::to_string(i), false));
    ...
    expert_down_bias_indices.push_back(context.requestWeight(
      ...
      weight_initializer, weight_regularizer, weight_regularizer_constant,
      weight_decay, "expert_down_bias_" + std::to_string(i), false));
  }
...
inline void GptOssMoELayer::compute_expert_forward(
...
inline void GptOssMoELayer::compute_expert_forward_no_critical(
  const nntrainer::Tensor &input, nntrainer::Tensor &expert_output,
  const std::vector<std::pair<unsigned, float>> &token_assignments,
  const nntrainer::Tensor &gate_proj, const nntrainer::Tensor &up_proj,
  const nntrainer::Tensor &down_proj, unsigned int hidden_size,
  const nntrainer::Tensor &gate_bias, const nntrainer::Tensor &up_bias,
...
void GptOssMoELayer::setProperty(const std::vector<std::string> &values) {
  auto remain_props = loadProperties(values, moe_props);
  nntrainer::LayerImpl::setProperty(remain_props);
}

void GptOssMoELayer::calcDerivative(nntrainer::RunLayerContext &context) {
...
void GptOssMoELayer::calcGradient(nntrainer::RunLayerContext &context) {
  // MoE layer does not support gradient calculation
  throw std::runtime_error("MoE layer does not support gradient calculation");
}

void GptOssMoELayer::exportTo(nntrainer::Exporter &exporter,
