...
class Ernie4_5_MoeRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        print(f"RMS Norm output : {self.weight * hidden_states.to(input_dtype)}")
...
class Ernie4_5_MoeStatics(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        #   2. Isolate does not work on `nn.Module` which only has parametersi
        print(f"router's output : {hidden_states}")
        print(f"after add bias : {hidden_states + self.e_score_correction_bias.squeeze()}")
...
class Ernie4_5_MoeSparseMoeBlock(nn.Module):
    ...
    def forward(
        ...
        print(f"SparseMoe input : {hidden_states}")
        with torch.autocast(device_type=device_type, enabled=False):  # Force float32
            # router_logits: (batch * sequence_length, n_experts)
            router_logits = self.gate(hidden_states.float())
            print(f"before softmax router_logits : {router_logits}")
        ...
        print("selected expert : ", end="")
        for expert_idx in expert_hit:
            print(f"{expert_idx}, ", end ="")
        ...
        print(f"MoESparese output : {final_hidden_states}")
