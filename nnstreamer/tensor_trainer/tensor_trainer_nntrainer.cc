...
static void nntrainer_model_destructor(const GstTensorFilterProperties * prop, void **private_data)
{
  NNTrainer::NNTrainerTrain *nntrainer = static_cast<NNTrainer::NNTrainerTrain *>(*private_data);
...
static void nntrainer_model_train(const GstTensorFilterProperties * prop, void **private_data)
{
  NNTrainer::NNTrainerTrain *nntrainer = static_cast<NNTrainer::NNTrainerTrain *>(*private_data);
  if (!nntrainer) {
...
void NNTrainer::NNTrainerTrain::createModel() {
  ...
  try {
    ...
    std::cerr << "Failed loading model configuration file" << typeid(e).name() << e.what() <<std::endl;
  }
...
static int nntrainer_model_construct_with_conf(const GstTensorFilterProperties * prop, void **private_data)
{
  ...
  try {
    nntrainer = new NNTrainer::NNTrainerTrain("/home/hyunil/nnstreamer/mnist.ini");
  } catch (std::exception &e) {
    std::cerr << "Error" << typeid(e).name() <<","<<e.what() <<std::endl;
  }
...
static int nntrainer_model_construct(const GstTensorFilterProperties * prop, void **private_data)
{
...
static GstTensorFilterFramework NNS_Trainer_support_nntrainer = {
  ...
  .close = nntrainer_model_destructor,  //.destructor = nntrainer_model_destructor,
