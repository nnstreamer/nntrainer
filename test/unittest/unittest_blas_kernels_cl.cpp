...
DECLARE_q4_0_test_M_K_N(3072, 3072, 3072);
...
//   std::vector<float> activation = generate_random_vector<float, false>(M *
//   K); float *activations_f32_ptr = (float *)allocateSVM(M * K *
//   sizeof(float));
//   blas_cc->command_queue_inst_.enqueueSVMMap(activations_f32_ptr,
//                                              M * K * sizeof(float), false);
//   for (unsigned int i = 0; i < M * K; ++i) {
...
//   for (unsigned int i = 0; i < run_count; ++i) {
//     nntrainer::gemm_q4_0_cl(wq0, activations_f32_ptr, (float *)out0, M, N0,
//     K); nntrainer::gemm_q4_0_cl(wq1, activations_f32_ptr, (float *)out1, M,
//     N1, K); nntrainer::gemm_q4_0_cl(wq2, activations_f32_ptr, (float *)out2,
//     M, N1, K);
//   }
...
//   for (unsigned int i = 0; i < run_count; ++i) {
//     nntrainer::gemm_q4_0_async_cl(weight_vec, activations_f32_ptr, out_vec,
//     M,
//                                   n_vec, K);
//   }
//   auto t4 = std::chrono::high_resolution_clock::now();
//   auto gpu_dt = std::chrono::duration_cast<std::chrono::milliseconds>(t4 -
//   t3);

//   std::cout << "Q4_0 GEMM : " << M << " x " << K << " x " << N1 << std::endl;
//   std::cout << " - time : Orig = " << dt.count() / (run_count * 1.0f) << "
//   ms"
//             << std::endl;
