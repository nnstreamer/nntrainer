...
static void run_q_4_K_test(const uint32_t M, const uint32_t K,
  ...
  for (unsigned int i = 0; i < run_count; ++i) {
    if (M == 1) {
      nntrainer::sgemv_q4_k_cl(M, N, K, activations_f32_ptr,
                               q4_weight_repack_ptr, gpu_q4_dst);
    } else {
      nntrainer::sgemm_q4_k_cl(M, N, K, activations_f32_ptr,
                               q4_weight_repack_ptr, gpu_q4_dst);
    }
  }
  auto t4 = std::chrono::high_resolution_clock::now();
...
DECLARE_q_4_K_test_M_K_N(256, 1024, 512);

DECLARE_q_4_K_test_M_K_N(1024, 3072, 3072);
DECLARE_q_4_K_test_M_K_N(3072, 1024, 3072);
DECLARE_q_4_K_test_M_K_N(3072, 3072, 1024);
