...
static void run_q_4_K_test(const uint32_t M, const uint32_t K,
  ...
  static constexpr size_t sizeof_block_q4_K = 144;

  unsigned int blocks_per_row = (K + qk_k - 1) / qk_k;
  unsigned int qa_size = sizeof_block_q8_K * blocks_per_row;
  unsigned int B_step = sizeof_block_q4_K * (K / qk_k);
  ...
  auto t3 = std::chrono::high_resolution_clock::now();
  for (unsigned int i = 0; i < run_count; ++i) {
    nntrainer::sgemv_q4_k_cl(M, N, K, activations_f32_ptr, q4_weight_repack_ptr,
                             gpu_q4_dst);
  }
  auto t4 = std::chrono::high_resolution_clock::now();
  auto gpu_dt = std::chrono::duration_cast<std::chrono::milliseconds>(t4 - t3);
