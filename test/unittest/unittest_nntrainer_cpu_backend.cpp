...
TEST(nntrainer_cpu_backend_standalone, q4_K_quantization) {
  ...
  nntrainer::quantize_q4_K(rhs_ptr,
                           /*dst quantized vector*/ (void *)offline_qWeight_ptr,
                           K, N, nullptr);
...
static float test_gemm_q4_0(const uint32_t M, const uint32_t K,
  ...
  nntrainer::repack_q4_0_to_q4_0_8(
    q4_0_repacked_qWeight.data(), q4_0_offline_qWeight_ptr, q4_0_data_size,
    /*row*/ N, /*col*/ K); // W is transposed, so it is N*K ?
...
static float test_gemm_q4_K(const uint32_t M, const uint32_t K,
  ...
  // Step1. Supposed to be an offline Weight quantization from float to q4_K
  // (Zero latency overhead for the model runtime)
  nntrainer::quantize_q4_K(weights,
                           /*dst quantized vector*/ (void *)offline_qWeight_ptr,
                           N, K, /*imatrix*/ nullptr);
  ...
  nntrainer::repack_q4_K_to_q4_K_8(
    repacked_qWeight.data(), offline_qWeight_ptr, data_size, /*row*/ N,
    /*col*/ K); // W is transposed, so it is N*K ?
...
static void run_quant_test(const uint32_t M, const uint32_t K, const uint32_t N,
  ...
  auto t1 = high_resolution_clock::now();
  nntrainer::sgemm(/*ROW MAJOR*/ 0, false, true, M, N, K, 1.F,
                   activation.data(), K, weight.data(), K, 0.F, ref_dst.data(),
                   N);
  auto t2 = high_resolution_clock::now();
  auto dt = duration_cast<nanoseconds>(t2 - t1);
