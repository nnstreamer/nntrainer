 ...
 */
NNTR_API inline void throw_status(int status) {
  switch (status) {
  case ML_ERROR_NONE:
    break;
 ...
 * @param[in] x float
 */
template <typename T = float> NNTR_API T sqrtFloat(T x) {
 ...
 */
NNTR_API double sqrtDouble(double x);

/**
 * @brief     abs function for float type
 * @param[in] x float
 */
template <typename T = float> NNTR_API T absFloat(T x) {
 ...
 * @param[in] x float
 */
template <typename T = float> NNTR_API T logFloat(T x) {
 ...
 * @param[in] x float
 */
template <typename T = float> NNTR_API T exp_util(T x) {
  return static_cast<T>(exp(x));
}

#ifdef _WIN32
#ifdef _Float16
template <> NNTR_API _Float16 exp_util<_Float16>(_Float16 x) {
 ...
 * @note      We can switch to use std::isfinite once it will start support half
 floats
 */
template <typename T> NNTR_API bool isFloatValid(const T value) {
  return !((value != value) ||
 ...
 */
NNTR_API bool isFileExist(std::string file);
 ...
 */
NNTR_API void checkedRead(std::ifstream &file, char *array,
                          std::streamsize size,
 ...
 */
NNTR_API void checkedWrite(std::ostream &file, const char *array,
                           std::streamsize size,
                           const char *error_msg = default_error_msg);
/**
 ...
 */
NNTR_API std::string readString(std::ifstream &file,
                                const char *error_msg = default_error_msg);
 ...
 */
NNTR_API void writeString(std::ofstream &file, const std::string &str,
                          const char *error_msg = default_error_msg);
 ...
 */
NNTR_API bool endswith(const std::string &target, const std::string &suffix);
...
NNTR_API void printInstance(std::ostream &out, const T &t) {
 ...
 */
template <typename T>
NNTR_API T checkedOpenStream(const std::string &path,
 ...
 */
NNTR_API int getKeyValue(const std::string &input_str, std::string &key,
                         std::string &value);
 ...
 * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
 */
NNTR_API int getValues(int n_str, std::string str, int *value);
 ...
 */
NNTR_API std::vector<std::string> split(const std::string &s,
                                        const std::regex &reg);
 ...
 */
NNTR_API bool istrequal(const std::string &a, const std::string &b);
...
NNTR_API bool enum_class_logical_and(T e1, T e2) {
 ...
 */
template <typename T, typename C = int> NNTR_API T enum_class_or(T e1, T e2) {
  C i1 = static_cast<int>(e1);
  C i2 = static_cast<int>(e2);
...
NNTR_API std::string find_in_tuple(const Tuple &t, std::index_sequence<ls...>,
...
NNTR_API std::string find_in_tuple(const std::tuple<Args...> &t,
                                   const std::string &key) {
 ...
 */
NNTR_API char *getRealpath(const char *name, char *resolved);
 ...
 * @return tm struct
 */
NNTR_API tm *getLocaltime(tm *tp);
 ...
 * @return std::regex
 */
NNTR_API std::regex getRegex(const std::string &str);
 ...
 */
NNTR_API void floatToFixedPointAndExponent(float input, int &fixedpoint,
                                           int &exponent);
 ...
 */
NNTR_API float fixedPointAndExponentToFloat(int fixedpoint, int exponent);
