 ...
 */
template <typename T> NNTR_API void createIfNull(std::unique_ptr<T> &ptr) {
  if (ptr == nullptr) {
...
public:
   ...
   *
   */
  NNTR_API Exporter(flatbuffers::FlatBufferBuilder *fbb);

  NNTR_API flatbuffers::FlatBufferBuilder *getFlatbufferBuilder() {
    return fbb;
  }
#endif
  ...
  NNTR_API void saveResult(const std::tuple<Ts...> &props,
  ...
  NNTR_API std::unique_ptr<T> getResult();
...
private:
  ...
  NNTR_API void saveTflResult(const PropsType &props, const NodeType *self);
 ...
 */
template <typename PropsType, typename NodeType>
NNTR_API void Exporter::saveTflResult(const PropsType &props,
                                      const NodeType *self) {
...
typename std::enable_if<I == sizeof...(Ts), void>::type NNTR_API
...
NNTR_API std::vector<std::string>
