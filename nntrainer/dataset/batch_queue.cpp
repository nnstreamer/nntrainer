...
ScopedView<Sample> IterationQueue::requestEmpty() {
  ...
  NNTR_THROW_IF(flow_state != FlowState::FLOW_STATE_OPEN, std::invalid_argument)
    ...
    << static_cast<unsigned>(flow_state.load());
...
ScopedView<Iteration> IterationQueue::requestFilled() {
  ...
  if (iteration == nullptr) {
    auto old_flow_state = flow_state.exchange(FlowState::FLOW_STATE_STOPPED);
    NNTR_THROW_IF(old_flow_state != FlowState::FLOW_STATE_STOP_REQUESTED,
                  std::runtime_error)
