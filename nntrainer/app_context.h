...
public:
   ...
   */
  NNTR_EXPORT AppContext() : Context(std::make_shared<ContextData>()) {}

  /**
   * @brief   Default destructor
   */
  NNTR_EXPORT ~AppContext() override = default;

  /**
   * @brief Deleting copy constructor
   *
   */
  NNTR_EXPORT AppContext(const AppContext &) = delete;

  /**
   * @brief Deleting assignment operator
   *
   */
  NNTR_EXPORT AppContext &operator=(const AppContext &) = delete;

  /**
   * @brief Deleting move constructor
   *
   */
  NNTR_EXPORT AppContext(AppContext &&) = delete;

  /**
   * @brief Deleting move assignment operator
   *
   */
  NNTR_EXPORT AppContext &operator=(AppContext &&) = delete;
   ...
   * @return AppContext&
   */
  NNTR_EXPORT AppContext &Global() override;
   ...
   * invalid
   */
  NNTR_EXPORT int registerLayer(const std::string &library_path,
   ...
   * invalid
   */
  NNTR_EXPORT int registerOptimizer(const std::string &library_path,
                                    const std::string &base_path = "");
   ...
   */
  NNTR_EXPORT const std::string getWorkingPath(const std::string &path = "");

  /**
   * @brief Get memory fsu file path from configuration file
   * @return memory fsu path.
   * If memory fsu path is not presented in configuration file, it returns
   * empty string
   */
  NNTR_EXPORT const std::vector<std::string> getProperties(void);
   ...
   */
  template <typename T>
  NNTR_EXPORT const int registerFactory(const PtrFactoryType<T> factory,
   ...
   */
  template <typename T>
  NNTR_EXPORT const int registerFactory(const FactoryType<T> factory,
                                        const std::string &key = "",
                                        const int int_key = -1);
  ...
  NNTR_EXPORT std::unique_ptr<nntrainer::Optimizer> createOptimizerObject(
   ...
   */
  template <typename T>
  NNTR_EXPORT PtrType<T> createObject(const int int_key,
   ...
   */
  template <typename T>
  NNTR_EXPORT PtrType<T> createObject(const std::string &key,
                                      const PropsType &props = {}) const {
    auto &index = std::get<IndexType<T>>(factory_map);
   ...
   */
  template <typename T>
  NNTR_EXPORT static PtrType<T> unknownFactory(const PropsType &props) {
    throw std::invalid_argument("cannot create unknown object");
  }

  NNTR_EXPORT std::string getName() override { return "cpu"; }

  NNTR_EXPORT void setMemAllocator(std::shared_ptr<MemAllocator> mem) {
 ...
 */
extern template NNTR_EXPORT const int
