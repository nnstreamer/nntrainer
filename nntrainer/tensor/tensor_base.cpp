...
void TensorBase::allocateInternal() {
  const size_t data_bytes_size = getDataBytesSize();
  const size_t scale_bytes_size = getScaleBytesSize();
  const size_t zero_point_bits_size = getZeroPointBytesSize();

  const size_t total_bytes_size =
    data_bytes_size + scale_bytes_size + zero_point_bits_size;

  data = std::make_shared<MemoryData>(
    std::aligned_alloc(TENSOR_DATA_ALIGNMENT, total_bytes_size), true);
}
