...
void ShortTensor::allocate() {
  ...
  if (src_tensor) {
    ...
    data = std::shared_ptr<MemoryData>(mem_data, [](auto *ptr) {
      delete[] ptr->template getAddr<int16_t>();
      delete ptr;
    });
...
float ShortTensor::max_abs() const {
  ...
  int16_t max_val = _data[0];
  for (unsigned int i = 1; i < size(); i += 1) {
    int16_t cur_val = (_data[i] >= 0) ? _data[i] : -1 * _data[i];
    if (cur_val > max_val) {
      max_val = cur_val;
    }
  }
