...
// should add fallback for incX incY + no functions for cblas
#include <neon_simd.h>
#elif USE_AVX
// avx_simd : avx based hgemm / hgemv / cpy_f32f16f32 + cblas
#include <avx_simd.h>
// #include <cblas_interface.h> 이거를 내부적으로 사용해야할듯?(sgemm / hgemm)
#include <custom_fallback.h>
#elif USE_BLAS
#include <cblas_interface.h>
#include <custom_fallback.h>
// since we have avx-based hgemm solution, custom_fallback should NOT have hgemm
// related function
#include <tmp_fallback.h>
#else
// cblas_fallback : only has counter-cblas fallbacks
#include <cblas_fallback.h>
// custom_fallback : everything except for cblas + hgemm + hgemv + cpy_f32_f16 /
// f16_f32
#include <custom_fallback.h>
// tmp_fallback : hgemm + hgemv + cpy_f32_f16 / f16_f32
#include <tmp_fallback.h>
// note that for ele_ops, we should have 'singular' function name!
// should explicitly implement for incX / incY > 1 case for archs
#endif
 ...
 * @param[in] X __fp16 * for Vector X
 */
extern _FP16 snrm2(const unsigned int N, const _FP16 *X, const int incX);
 ...
 */
extern void scopy(const unsigned int N, const _FP16 *X, const int incX,
 ...
 * @param[in] Y __fp16 * for Vector Y
 */
extern void scopy(const unsigned int N, const float *X, const int incX,
                  _FP16 *Y, const int incY);
 ...
 */
extern void sgemm(const unsigned int TStorageOrder, bool TransA, bool TransB, const unsigned int M,
 ...
 * @param[in] beta float number
 */
extern void sgemv(const unsigned int TStorageOrder, bool TransA, const unsigned int M, const unsigned int N,
                  const float alpha, const _FP16 *A, const unsigned int lda,
                  const _FP16 *X, const int incX, const float beta, _FP16 *Y,
                  const int incY);
/**
 ...
 * @param[in] X float * for Vector X
 */
extern float snrm2(const unsigned int N, const float *X, const int incX);
 ...
 */
extern void scopy(const unsigned int N, const uint8_t *X, const int incX,
...
extern void sgemm(const unsigned int TStorageOrder, bool TransA, bool TransB, const unsigned int M,
                  const unsigned int N, const unsigned int K, const float alpha,
                  const float *A, const unsigned int lda, const float *B,
