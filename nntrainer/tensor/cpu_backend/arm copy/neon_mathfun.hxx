...
void sincos_ph(float16x8_t x, float16x8_t* ysin, float16x8_t* ycos) {
  ...
  // floorf emulation in f16:
  // tmpi = (int16) y; tmpf = (float16) tmpi; if (tmpf > y) tmpi -= 1;
  ...
  // polynomial selection mask: bit 1 of emm2 (i.e., (emm2 & 2) != 0)
  uint16x8_t poly_mask = vtstq_u16(emm2, vdupq_n_u16(2));

  // sine/cosine sign masks:
  // sin flips sign when (emm2 & 4) != 0
  sign_mask_sin = veorq_u16(sign_mask_sin, vtstq_u16(emm2, vdupq_n_u16(4)));
  // cos sign is negative when ((emm2 - 2) & 4) != 0 -> same bit-test trick
  ...
  // y1 = cos poly (even), y2 = sin poly (odd)
