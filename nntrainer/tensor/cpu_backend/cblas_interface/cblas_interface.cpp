...
void sgemv(const unsigned int TStorageOrder, bool TransA, const unsigned int M, const unsigned int N,
  ...
  openblas_set_num_threads(BLAS_NUM_THREADS);
#endif
  return cblas_sgemv(order, transA, M, N, alpha, A, lda, X, incX, beta,
                     Y, incY);
}
...
void sgemm(const unsigned int TStorageOrder, bool TransA, bool TransB, const unsigned int M, const unsigned int N,
