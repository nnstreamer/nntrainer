...
void __fallback_hsgemv(const unsigned int TStorageOrder, bool TransA,
  ...
  void __fallback_compute_rotary_embedding_value(
    unsigned int dim, unsigned int half_, unsigned int w, _FP16 *in, _FP16 *out,
  ...
  void __fallback_softmax(const unsigned int N, _FP16 *X, _FP16 *Y) {
    ...
    while (i < N) {
      Y[i] =
        static_cast<_FP16>(std::exp(static_cast<float>(X[i] - max_x)) / sum);
      ++i;
    }
  }
  ...
  void __fallback_sgemv(
    const unsigned int TStorageOrder, bool TransA, const unsigned int M,
