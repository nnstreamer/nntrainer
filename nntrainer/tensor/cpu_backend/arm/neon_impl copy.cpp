...
static __fp16 hsumq_f16(float16x8_t v) {
  ...
  float16x4_t s4 = vadd_f16(lo, hi);                  // [0+4,1+5,2+6,3+7]
  float16x4_t s2 = vadd_f16(s4, vext_f16(s4, s4, 2)); // [0+2,1+3,*,*]
  float16x4_t s1 = vadd_f16(s2, vext_f16(s2, s2, 1)); // [0+1+2+3,*,*,*]
  return vget_lane_f16(s1, 0);
}

static void rms_norm_fp32_api_fp16_compute(const float *__restrict X,
                                           float *__restrict Y, std::size_t H,
                                           std::size_t W, float epsilon) {
  ...
  for (std::size_t h = 0; h < H; ++h) {
    ...
    // ---- Pass 1: sum of squares along width (FP16 accumulate) ----
    std::size_t i = 0;
    float16x8_t acc = vdupq_n_f16(0.F);

    // 8 elems/iter: load f32 -> convert to f16 -> accumulate h*h
    for (; i + 8 <= W; i += 8) {
...
// FULL FP32 RMS NORM
static void rms_norm_width_fp32(const float *__restrict X, float *__restrict Y,
                                std::size_t H, std::size_t W, float epsilon) {
  for (std::size_t h = 0; h < H; ++h) {
    const float *rowX = X + h * W;
    float *rowY = Y + h * W;
...
void rms_norm_width_fp32_strided(const float *__restrict X, float *__restrict Y,
                                 std::size_t H, std::size_t W, float epsilon) {
  rms_norm_fp32_api_fp16_compute(X, Y, H, W, epsilon);
}
