/**
 * @file kai_rhs_pack_kxn_qsi4cxp_qs4cxs1s0.h
 * @date   22 Septemer 2025
 * @see    https://github.com/ARM-software/kleidiai
 * @author Sungsik Kong <ss.kong@samsung.com>
 * @bug    No known bugs except for NYI items
 * @brief  kai_rhs_pack_kxn_qsi4cxp_qs4cxs1s0.h
...
///
/// @note  The int4 values are stored in a K x N matrix. Two int4 values are
/// stored in one byte.
///        The lower order part of the byte (low) holds the first nibble
///        (K-index + 0). The higher order of the byte holds the second nibble
///        (K-index + 1).
///
...
/// packed 4-bit quantized symmetric per-channel (qsu4cx) values.
///
/// @param[in] n_idx Row index in the RHS matrix (not packed). It must be a
/// multiple of n_step.
/// @param[in] k     In the RHS matrix (not packed), K is the number of rows.
...
/// @param[in]  nr          The number of N columns to interleave on the same
/// output output row.
