...
class nntr_tensor_traits {
  public:
    virtual ~nntr_tensor_traits();
    virtual bool work_size(int n_threads, const struct ggml_tensor * op, size_t & size)        = 0;
    virtual bool compute_forward(struct ggml_compute_params * params, struct ggml_tensor * op) = 0;
};

class tensor_traits_base : public nntr_tensor_traits {
  public:
...
static const tensor_traits * ggml_get_optimal_repack_type(unsigned int row_size) {
    ...
    //     }
    // }
    // TODO: implement/divide above logic arch-specific
    if (ggml_cpu_has_avx2()) {
        if (row_size % 8 == 0) {
...
void nntr_q4_K_8x8_q8_K_GEMM(unsigned int row_size) {
    auto gemm_fn = ggml_get_optimal_repack_type(row_size);
    gemm_fn->compute_forward(nullptr, nullptr);
}
