        ...
        for (unsigned int num_w = 0; num_w < Ns.size(); ++num_w) {
          ...
          if (N > 256) {
            ...
            ::ggml_gemv_q4_K_8x8_q8_K(
              ...
              (void *)((char *)B + M_step_start * B_step), QA.data(), M,
              M_step_end - M_step_start);
          }
        }
      });
...
//   // Subtract 1 to handle cases where N is already a power of two
//   N--;
...
void __ggml_gemm_q6_K(const unsigned int M, const unsigned int N,
  ...
  */
  ...
  // }
    ...
    const int grain_rows = (M + num_threads - 1) / num_threads;

    auto fut = tp.submit_loop(0, num_threads, [&](int t) {
      const int i0 = t * grain_rows;
      const int i1 = std::min<int>(M, i0 + grain_rows);
      for (int i = i0; i < i1; ++i) {
        const void *a_row = quantized_A.data() + i * A_row_size;
        float *c_row = C + i * ldc;
        for (unsigned int j = 0; j < N; ++j) {
