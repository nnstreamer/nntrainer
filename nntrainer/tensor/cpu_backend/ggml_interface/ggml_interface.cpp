...
//     }
    ...
    BS::multi_future<void> multi_future = bspool.submit_loop(0, n_threads, [&](int i){::ggml_gemv_q4_K_8x8_q8_K(K, (float *)((C) + (i * N) / n_threads), N,
                                (void *)((char *)B + (i * N) / n_threads * B_step),
                                QA.data(), M, ((i + 1) * N) / n_threads - (i * N) / n_threads);});
    multi_future.wait();
