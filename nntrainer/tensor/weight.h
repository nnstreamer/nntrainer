...
public:
   ...
   */
  NNTR_API explicit Weight(const Spec &spec, bool alloc_now = false) :
   ...
   */
  NNTR_API explicit Weight(const Tensor &v, const Tensor &g, const Tensor &v32,
   ...
   * @param reg_const Constant multiplier for regularizer
   */
  NNTR_API explicit Weight(Tensor *v, Tensor *g, Tensor *v32,
   ...
   */
  NNTR_API friend void swap(Weight &lhs, Weight &rhs) noexcept {
    using std::swap;
   ...
   */
  NNTR_API Weight(const Weight &rhs) = default;
   ...
   * @param rhs weight to construct from
   */
  NNTR_API Weight(Weight &&rhs) = default;
   ...
   */
  NNTR_API Weight &operator=(const Weight &rhs) = default;
   ...
   * @return Weight& Updated weight
   */
  NNTR_API Weight &operator=(Weight &&rhs) = default;
   ...
   */
  NNTR_API void clearOptimizerVariables() { opt_vars.clear(); }

  /**
   * @brief Add optimizer variables
   * @param dim Optimizer variable dimension
   */
  NNTR_API void setOptimizerVariables(std::vector<Tensor *> tensors) {
    opt_vars = tensors;
  }
   ...
   */
  NNTR_API Tensor &getOptimizerVariableRef(unsigned int idx) {
    return *opt_vars[idx];
  }
   ...
   */
  NNTR_API int getNumOptVariable() { return opt_vars.size(); }

  /**
   * @brief Get axis of Weight
   * @retval axis of Wegiht
   */
  NNTR_API unsigned int getOutputAxis() { return output_axis; }

  /**
   * @brief     check if weight regularizer type is l2norm
   * @return    bool is weight regrulatizer type is L2 Norm
   */
  NNTR_API bool isWeightRegularizerL2Norm() {
   ...
   */
  NNTR_API bool isWeightDecay() { return decay > epsilon_decay; }

  /**
   * @brief     Get loss from the regularization of the weight
   */
  NNTR_API float getRegularizationLoss() {
   ...
   */
  NNTR_API void calcRegularizationGradient() {
   ...
   */
  NNTR_API void calcWeightDecayGradient() {
    if (isWeightDecay())
      applyWeightDecay();
  }
   ...
   */
  NNTR_API void applyGradient(double lr) { var->add_i(*grad.get(), -lr); }
   ...
   */
  NNTR_API void applyGradient(double lr, Tensor &updated_grad);
   ...
   */
  NNTR_API static bool isGradientClipByGlobalNorm(const float max_norm) {
   ...
   * @return false otherwise
   */
  NNTR_API bool isGradientClipByGlobalNorm() const {
   ...
   * @return false otherwise
   */
  NNTR_API bool isMixedPrecision() const { return is_mixed; }
   ...
   */
  NNTR_API void clipGradientByGlobalNorm(const float global_norm) {
   ...
   */
  NNTR_API Tensor &getVariableFP32Ref() { return *var32.get(); }

  /**
   * @brief Quantize var32 to var
   *
   */
  NNTR_API void quantizeWeight();
   ...
   *
   */
  NNTR_API void setLossScale(float scale) { loss_scale = scale; };

  /**
   * @brief get loss scale
   *
   */
  NNTR_API const float getLossScale() { return loss_scale; };
