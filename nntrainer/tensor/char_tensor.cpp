...
bool CharTensor::operator==(const CharTensor &rhs) const {
  ...
  for (size_t i = 0; i < size(); ++i) {
    if (std::isnan(_data[i]) || std::isnan(_rdata[i]) || _data[i] != _rdata[i])
      return false;
  }
...
void CharTensor::setRandNormal(float mean, float stddev) {
  throw std::invalid_argument(
    "[CharTensor::setRandNormal] Currently unsupported in QINT8 data type");
}

void CharTensor::setRandUniform(float min, float max) {
  throw std::invalid_argument(
    "[CharTensor::setRandUniform] Currently unsupported in QINT8 data type");
}

void CharTensor::setRandBernoulli(float probability) {
  throw std::invalid_argument(
    "[CharTensor::setRandBernoulli] Currently unsupported in QINT8 data type");
}
...
Tensor CharTensor::multiply_strided(Tensor const &m, Tensor &output,
                                    const float beta) const {
  return output;
}

int CharTensor::multiply_i(float const &value) { return ML_ERROR_NONE; }

Tensor &CharTensor::multiply(float const &value, Tensor &out) const {
  return out;
}

Tensor &CharTensor::multiply(Tensor const &m, Tensor &output,
                             const float beta) const {
  return output;
}

Tensor &CharTensor::divide(float const &value, Tensor &output) const {
  return output;
}

Tensor &CharTensor::divide(Tensor const &m, Tensor &output) const {
  return output;
}

Tensor &CharTensor::add_strided(Tensor const &input, Tensor &output,
                                const float beta) const {
  return output;
}

int CharTensor::add_i(Tensor const &m, Tensor &output, float const alpha) {
  return ML_ERROR_NONE;
}

Tensor &CharTensor::add(float const &value, Tensor &output) const {
  return output;
}

Tensor &CharTensor::add(Tensor const &m, Tensor &output,
                        float const alpha) const {
  return output;
}

Tensor &CharTensor::subtract(float const &value, Tensor &output) const {
  return output;
}

void CharTensor::sum_by_batch(Tensor &output) const { return; }

Tensor &CharTensor::sum(unsigned int axis, Tensor &output, float alpha,
                        float beta) const {
  return output;
}

float CharTensor::l2norm() const { return 0.0f; }

Tensor &CharTensor::pow(float exponent, Tensor &output) const { return output; }

Tensor &CharTensor::erf(Tensor &output) const { return output; }

Tensor &CharTensor::dot(Tensor const &input, Tensor &output, bool trans,
                        bool trans_in, float beta) const {
  return output;
}

void CharTensor::dropout_mask(float dropout) {}

void CharTensor::filter_mask(const Tensor &mask_len, bool reverse) {}

void CharTensor::zoneout_mask(Tensor &opposite, float zoneout) {}

std::vector<Tensor> CharTensor::split(std::vector<size_t> sizes, int axis) {
...
void CharTensor::copyData(const Tensor &from) {
  ...
  /// @todo support fp32 & fp16 to int8
...
Tensor &CharTensor::transpose(const std::string &direction,
                              Tensor &output) const {
  return output;
}
