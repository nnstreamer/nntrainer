...
__kernel void rotary_emb_cl_fp16(
  __global half *input, __global half *output, __global float *freqs_cos,
  __global float *freqs_sin, __global float *cos_, __global float *sin_,
  unsigned int batch, unsigned int channel, unsigned int height,
  unsigned int width, unsigned int dim, unsigned int half_,
  unsigned int max_timestep, unsigned int from, unsigned int offsetFreqsSin,
    ...
    for (unsigned int h = 0; h < height; h++) {
      ...
      for (unsigned int w = 0; w < width; w = w + dim) {
        for (unsigned int k = 0; k < dim; k++) {
          ...
          value = (float)input[b * channel * height * width +
                               c * height * width + h * width + span];
          if (k < half_) {
            transformed_value =
              -1.0f *
              (float)input[b * channel * height * width + c * height * width +
                           h * width + span + half_];
          } else {
            transformed_value =
              (float)input[b * channel * height * width + c * height * width +
