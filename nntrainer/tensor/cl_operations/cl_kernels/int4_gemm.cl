...
// Modifications made by Donghyeon Jeong on September 20 2025:
// - Only support fc_bf_tiled_kernel_dyn_quan kernel
...
#define TO_INPUT0_TYPE(x) CAT(convert_, INPUT0_TYPE)(x)
...
#define TILE_IFM 2
#define OUTER_OFM 1
#define HALF_TILE_B (TILE_B / 2)
#define TILE_IFM_ELEMENTS_SIZE (TILE_IFM * SIMD)
#define QUANTIZE_GROUP_SIZE 128

#if COMPRESSED_WEIGHTS_INT4
#define TILE_K_OFM_PACKED (TILE_K_OFM / 2)
#else
#define TILE_K_OFM_PACKED TILE_K_OFM
#endif
...
void kernel quantize_input(const __global INPUT0_TYPE *input,
...
inline void kernel fc_bf_tiled_kernel_dyn_quan(
  __global DQ_TYPE *quantized_input,
  ...
  __local uint wei_local_mem[SIMD * TILE_OFM * SIMD];

  const int INPUT_ELEMENTS_COUNT = M * K;
  const int MAIN_LOOP_ELEMENTS_COUNT = INPUT_ELEMENTS_COUNT;

  const int TILE_OUT_F_NUM = N;
  ...
  uint input_offset = out_b * TILE_IN_B_PITCH;
  ...
  __attribute__((opencl_unroll_hint(1))) for (uint oi = 0; oi < OUTER_OFM;
    ...
    unroll_for(uint fi = 0; fi < TILE_OFM; ++fi) { wei_zp[fi] = 0; }
#endif
...
// iteration.
    ...
    uint weights_idx =
      ...
      dq_wei_unpacked.s4567 = wei_packed.s1357;
        ...
        // Calculate zero-point and scale only for
        // DQ_DECOMPRESSION_SCALE_POST_OP enabled Calculate weight : w = (w -
        // dzp) * ds if DECOMPRESSION_ZP_TERM is not enabled, then dzp is
...
#error "FC bf_tiled kernel: unsupported TILE_K size for SLM kernel"
  ...
  (TILE_IFM_ELEMENTS_SIZE <= DECOMPRESSION_SCALE_GROUP_SIZE)
    ...
    // =====================================================================================================================================
    // Write results
    uint output_offset = out_f * TILE_OUT_F_PITCH + out_b * TILE_OUT_B_PITCH;
