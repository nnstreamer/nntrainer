...
kernel void quantize_input_new(const __global half *restrict input,
...
kernel void quantize_input_pad(const __global half *restrict input,
                               __global char *restrict quantized_input,
                               __global half *restrict quan_var) {
  ...
  const uint output_offset = group_id * QUANTIZE_GROUP_SIZE;
  const uint max_quantize_block = QUANTIZE_GROUP_SIZE / 4;
  uint quantize_block;

  if (group_id_in_row == groups_in_row - 1) {
    quantize_block = (QUANTIZE_GROUP_SIZE - (SIZE_K - SIZE_K_ORIG)) / 4;
  } else {
    quantize_block = QUANTIZE_GROUP_SIZE / 4;
  }
...
#else
            const uint scale_offset =
              (offset_ofm % DECOMPRESSION_SCALE_BATCH_NUM) + ni_offset * SIZE_N;
#endif
            ACCUMULATOR_TYPE ds = decompression_scale[scale_offset];
#else
#if OUTER_OFM > 1
