...
const std::string &getQ4_0MulMatKernel() {
  static const std::string q4_0_fp32_mul_mat_kernel_ =
    R"(
    #pragma OPENCL EXTENSION cl_khr_fp16 : enable
    ...
    // mul_vec_q_n_f32
    //------------------------------------------------------------------------------
    // function for calculate inner product between half a q4_0 block and 16 floats
    // (yl), sumy is SUM(yl[i]) il indicates where the q4 quants begin (0 or
    // QK4_0/4) we assume that the yl's have been multiplied with the appropriate
    // scale factor that corresponds to the missing bit shifts (1, 1/16, 1/256,
    // 1/4096)
    inline float block_q_4_0_dot_y(global struct block_q4_0 *qb_curr, float sumy,
                                  private float *yl, int il) {
      float d = qb_curr->d;
      float2 acc = 0.f;
      global ushort *qs = ((global ushort *)qb_curr + 1 + il / 2);
      for (int i = 0; i < 8; i += 2) {
        acc.s0 +=
          yl[i + 0] * (qs[i / 2] & 0x000F) + yl[i + 1] * (qs[i / 2] & 0x0F00);
        acc.s1 +=
          yl[i + 8] * (qs[i / 2] & 0x00F0) + yl[i + 9] * (qs[i / 2] & 0xF000);
      }
    ...
    inline void mul_vec_q_n_f32(global void *src0, global float *src1,
                                global float *dst, int ne00, int ne01, int ne02,
                                int ne10, int ne12, int ne0, int ne1, int r2,
                                int r3) {
      ...
      ulong offset0 =
        first_row * nb + (i12 / r2) * (nb * ne01) + (i13 / r3) * (nb * ne01 * ne02);
    ...
    #endif
    kernel void kernel_mul_mat_q4_0_f32(global void *src0, global float *src1,
                                        global float *dst, int ne00, int ne01,
                                        int ne02, int ne10, int ne12, int ne0,
                                        int ne1, int r2, int r3) {
      mul_vec_q_n_f32(src0, src1, dst, ne00, ne01, ne02, ne10, ne12, ne0, ne1, r2,
                      r3);
    }
    )";
  return q4_0_fp32_mul_mat_kernel_;
}
