...
const std::string &getQ4KSgemmClKernel() {
  static const std::string q4_k_sgemm_cl_kernel =
    R"(
...
__kernel void  ggml_gemm_q4_K_8x8_q8_K(
    ...
    __global const block_q8_Kx4 * __restrict vy, 
             const int                       nr,
             const int                       nc) 
{
  const int qk = QK_K;
  ...
  __global const block_q8_Kx4 *a_ptr = vy + (y * nb);
  __global const block_q4_Kx8 *b_ptr = vx + (x * nb);
  ...
  for (int l = 0; l < nb; l++)
  {
    ...
    {
      const uint3 u012 = vload3(0, (__global const uint *)(b_ptr->scales + sb * 12));
      ...
      for (int m = 0; m < 4; m++)
      {
        for (int j = 0; j < NCOLS_INTERLEAVED; j++)
        {
          ...
          for (int i = 0; i < BLOCKLEN; ++i) {
            const int v0 = (char)(b_ptr[l].qs[k * NCOLS_INTERLEAVED * BLOCKLEN +
                                              j * BLOCKLEN + i] &
                                  0xF);
            const int v1 = (char)(b_ptr[l].qs[k * NCOLS_INTERLEAVED * BLOCKLEN +
                                              j * BLOCKLEN + i] >>
                                  4);
            sumi1 = (v0 * a_ptr[l].qs[(k >> 2) * 256 + (k % 4) * 4 * BLOCKLEN +
                                      m * BLOCKLEN + i]);
            sumi2 = (v1 * a_ptr[l].qs[(k >> 2) * 256 + (k % 4) * 4 * BLOCKLEN +
                                      m * BLOCKLEN + i + 128]);
    ...
    for (int sb = 0; sb < 8; sb++)
    {
      const uchar *mins = (const uchar *)utmp + 8 + sb * 16;

      for (int m = 0; m < 4; m++)
      {
        __global const short *bsums =  a_ptr[l].bsums + (sb * 8) + (m * 4) - ((sb % 2) * 6);
        for (int j = 0; j < NCOLS_INTERLEAVED; j++) {
