...
void gemm_q4_0_cl(const unsigned int M, const unsigned int N,
  ...
  kernel_q4_0_mul_mat_ptr =
    blas_cc->registerClKernel(getQ4_0MulMatKernel(), "kernel_mul_mat_q4_0_f32");

  if (!kernel_q4_0_mul_mat_ptr) {
    ml_loge("Failed to register kernel_q4_0_mul_mat_ptr");
    return;
  }

  // const size_t q4_0_bytes = sizeof(block_q4_0) * M * N / 32;

  blas_cc->command_queue_inst_.enqueueSVMUnmap(matAdata);

  blas_cc->command_queue_inst_.enqueueSVMUnmap(matBdata);
  ...
  block_q4_0 *A =
    static_cast<block_q4_0 *>(matAdata); // cast to block_q4_0 pointer

  result = kernel_q4_0_mul_mat_ptr->SetKernelSVMArguments(arg++, A);
  if (!result)
  ...
  const int work_groups_count[3] = {(ne01 + ndst - 1) / ndst * nth0,
                                    ne11 * nth1, ne12 * ne13};
  /// @todo: create a group size by device & input
