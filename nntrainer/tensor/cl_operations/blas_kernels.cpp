  ...
  for (unsigned int i = 0; i < Ns.size(); ++i) {
    ...
    result =
      kernel_ptr->SetKernelSVMArguments({{arg++, clbuffInstance.getSVMQuant(i)},
                                         {arg++, clbuffInstance.getSVMScale(i)},
    ...
    result =
      kernel_ptr->SetKernelArguments({{arg++, &N, sizeof(unsigned int)},
                                      {arg++, &padded_M, sizeof(unsigned int)},
                                      {arg++, &K, sizeof(unsigned int)},
                                      {arg++, &M, sizeof(unsigned int)}});
