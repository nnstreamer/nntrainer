...
void sgemm_q4_k_cl(const unsigned int M, const unsigned int N,
  ...
  ClContext::SharedPtrClKernel kernel =
    blas_cc->registerClKernel(getQ4KSgemmClKernel(), "ggml_gemm_q4_K_8x8_q8_K");

  if (!kernel) {
    ml_loge("Failed to register ggml_gemm_q4_K_8x8_q8_K");
    return;
  }

  if (!blas_cc->command_queue_inst_.enqueueSVMUnmap(A)) {
    ml_loge(
      "Failed to write data to input buffer A for ggml_gemm_q4_K_8x8_q8_K");
    return;
  }

  if (!blas_cc->command_queue_inst_.enqueueSVMUnmap(B)) {
    ml_loge(
      "Failed to write data to input buffer B for ggml_gemm_q4_K_8x8_q8_K");
    return;
  }
  ...
  if (!kernel->SetKernelArguments(2, &N, sizeof(unsigned int))) {
    ml_loge("Failed to set argument : %s", __LINE__);
    return;
  }
  if (!kernel->SetKernelSVMArguments(3, B)) {
    ml_loge("Failed to set argument : %s", __LINE__);
    return;
  }
  if (!kernel->SetKernelSVMArguments(4, A)) {
    ml_loge("Failed to set argument : %s", __LINE__);
    return;
  }
  if (!kernel->SetKernelArguments(5, &M, sizeof(unsigned int))) {
    ml_loge("Failed to set argument : %s", __LINE__);
    return;
  }
  if (!kernel->SetKernelArguments(6, &N, sizeof(unsigned int))) {
    ml_loge("Failed to set argument : %s", __LINE__);
    return;
  }
  ...
  if (!opencl::CommandQueueManager::GetInstance().DispatchCommand(
    ...
    ml_loge("Failed to dispatch kernel ggml_gemm_q4_K_8x8_q8_K");
    return;
  }
...
void sgemv_q4_k_cl(const unsigned int M, const unsigned int N,
  ...
  const int work_groups_count[3] = {(int)N / 8, 8, 1};
  const int work_group_size[3] = {4, 8, 1};

  if (!opencl::CommandQueueManager::GetInstance().DispatchCommand(
        kernel, work_groups_count, work_group_size)) {
    ml_loge("Failed to dispatch kernel ggml_gemv_q4_K_8x8_q8_K");
    return;
  }

  if (!blas_cc->command_queue_inst_.enqueueSVMMap(C, N * sizeof(float), true)) {
    ml_loge(
      "Failed to read data from the output buffer for ggml_gemv_q4_K_8x8_q8_K");
