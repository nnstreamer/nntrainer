...
public:
   ...
   */
  NNTR_EXPORT DynamicTrainingOptimization(float threshold_ = 1.0f,
                                          int skip_n_iter = 1);

  /**
   * @brief     Set threshold for optimization
   */
  NNTR_EXPORT void setThreshold(float threshold_) {
   ...
   */
  NNTR_EXPORT void setOp(const std::string &op) {
    if (op == dft_opt_max)
   ...
   */
  NNTR_EXPORT void enable() { enabled = true; }

  /**
   * @brief     Disable the optimization
   */
  NNTR_EXPORT void disable() { enabled = false; }

  /**
   * @brief     Set the mode for optimization
   */
  NNTR_EXPORT void setMode(const std::string &mode_) {
   ...
   */
  NNTR_EXPORT bool isDerivativeMode() {
   ...
   */
  NNTR_EXPORT bool isGradientMode() {
   ...
   *
   */
  NNTR_EXPORT void setSkipIterations(int skip_n_iter) {
   ...
   */
  NNTR_EXPORT bool checkIfApply(const std::vector<Weight> &weights,
   ...
   * @note true if should be applied, else false
   */
  NNTR_EXPORT bool checkIfApply(const Weight &weight,
                                const std::shared_ptr<Var_Grad> &input,
                                const std::shared_ptr<Var_Grad> &output,
                                const std::shared_ptr<OptimizerWrapped> &opt,
                                int iteration);
...
private:
   ...
   * @note true if should be applied, else false
   */
  NNTR_EXPORT bool checkIfApply(float reduced_ratio, float learning_rate);

  /**
   * @brief     Operation to decide if update should be skipped
   * @note      Calculate l0 norm of the tensor
   */
  NNTR_EXPORT static float reduceByMax(Tensor const &ratio);

  /**
   * @brief     Operation to decide if update should be skipped
   * @note      Calculate l2 norm of the tensor averaged by its size
   */
  NNTR_EXPORT static float reduceByNorm(Tensor const &ratio);
};
