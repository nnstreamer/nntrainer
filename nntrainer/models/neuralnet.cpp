...
/**
 * @brief     get Centroids function computes the centroid feature vector of
 * simpleshot classes which is added
 */
...
nntrainer::Tensor NeuralNetwork::getCentroids() {
  ...
  if (status == 0)
    std::cout << "Centroid Tensor :-> " << centroid_tensor << "\n";

  return centroid_tensor;
}

/**
 * @brief  predict function Predicts the accuracy of validation set using
 * nearest neighbour with centroid tensors
 */

void NeuralNetwork::predict(int earlier_classes, int tot_class,
   ...
   * @param on_epoch_end function that will recieve reference to stat,
   * buffer which will be called on the epoch end
   * @param earlier_classes earlier classes which were already present
   * @param tot_class total number of classes including earlier classes and
   * newly added classes
   * @param label_path label of all the classes present
   */

  auto run_epoch = [this, &in, &label, &in_dims, &label_dims,
                    batch_size](DataBuffer *buffer, bool shuffle,
                                auto &&on_iteration_fetch, auto &&on_epoch_end,
                                int earlier_classes, int tot_class,
                                std::string label_path) {
    /// @todo managing metrics must be handled here as well!! for now it is
    /// handled in individual callbacks
    RunStats stat;
    ...
    while (true) {
      ...
      on_iteration_fetch(stat, *buffer, V, all_labels_map, label_map, tot_class,
                         flag);
    }
  ...
  auto eval_for_iteration =
    [this, &output, &label, batch_size](
      RunStats &stat, DataBuffer &buffer, std::vector<nntrainer::Tensor> &V,
      std::map<int, std::string> &all_labels_map,
      std::map<int, std::string> &label_map, int tot_class, int flag) {
      forwarding(false);
      /// secondlast
      ...
      /**
    * @brief get distance calculates the L2 squared distance between two tensors
    ...
    */
      ...
      for (unsigned int b = 0; b < batch_size; b++) {
        ...
        if (flag == 0)
          s2 = label_map[label_out[b]];

        std::cout << "Class of Image: " << s1 << " Actual Class: " << s2
                  << std::endl;
  ...
  if (valid_buffer) {
    validation =
      run_epoch(valid_buffer.get(), false, eval_for_iteration, eval_epoch_end,
                earlier_classes, tot_class, label_path);
  }
  std::cout << '\n';
}
