...
public:
   ...
   */
  NNTR_EXPORT NeuralNetwork();

  /**
   * @brief     Constructor of NeuralNetwork Class
   */
  NNTR_EXPORT NeuralNetwork(const Engine *ct_engine_);
   ...
   * @retval    loss value
   */
  NNTR_EXPORT float getLoss() override;

  /**
   * @brief returns compilation state of a network
   * @retval initialized value
   */
  NNTR_EXPORT bool getCompiled() const override { return compiled; }

  /**
   * @brief returns initialization state of a network
   * @retval initialized value
   */
  NNTR_EXPORT bool getInitialized() const override { return initialized; }

  /**
   * @brief returns loadedFromConfig state of a network
   * @retval loadedFromConfig value
   */
  NNTR_EXPORT bool getLoadedFromConfig() const override {
   ...
   * @retval    loss value
   */
  NNTR_EXPORT float getTrainingLoss() override { return training.loss; }

  /**
   * @brief     Get Loss from the previous epoch of validation data
   * @retval    loss value
   */
  NNTR_EXPORT float getValidationLoss() override { return validation.loss; }

  NNTR_EXPORT RunStats getTrainingStats() override { return training; }

  NNTR_EXPORT RunStats getValidStats() override { return validation; }

  NNTR_EXPORT RunStats getTestStats() override { return testing; }
   ...
   */
  NNTR_EXPORT float getLearningRate() { return opt->getLearningRate(0); };
   ...
   */
  NNTR_EXPORT int loadFromConfig(const std::string &config) override;
   ...
   * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
   */
  NNTR_EXPORT int compile(ExecutionMode mode = ExecutionMode::TRAIN) override;
   ...
   */
  NNTR_EXPORT int
  initialize(ExecutionMode mode = ExecutionMode::TRAIN) override;
   ...
   * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
   */
  NNTR_EXPORT int reinitialize() override;
   ...
   * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
   */
  NNTR_EXPORT int allocate(ExecutionMode mode = ExecutionMode::TRAIN) override;
   ...
   */
  NNTR_EXPORT void inPlaceOptimization(const std::string &layer_type);
   ...
   */
  NNTR_EXPORT sharedConstTensors forwarding(sharedConstTensors input,
   ...
   * format);
   */
  NNTR_EXPORT void save(const std::string &file_path,
   ...
   * format);
   */
  NNTR_EXPORT void load(const std::string &file_path,
                        ml::train::ModelFormat format =
                          ml::train::ModelFormat::MODEL_FORMAT_BIN) override;
   ...
   * @retval    epochs
   */
  NNTR_EXPORT unsigned int getEpochs() {
   ...
   */
  NNTR_EXPORT unsigned int getCurrentEpoch() override;
   ...
   */
  NNTR_EXPORT NeuralNetwork &copy(NeuralNetwork &from);
   ...
   */
  NNTR_EXPORT NeuralNetwork &copyConfiguration(NeuralNetwork &from);
   ...
   */
  NNTR_EXPORT sharedConstTensors inference(sharedConstTensors X,
                                           bool free_mem = false);
   ...
   */
  NNTR_EXPORT std::vector<float *> inference(
   ...
   * @retval shared_ptr<const Tensor>
   */
  NNTR_EXPORT sharedConstTensors
   ...
   * @retval shared_ptr<const Tensor>
   */
  NNTR_EXPORT sharedConstTensors incremental_inference(
   ...
   */
  NNTR_EXPORT void resetInputDimension(std::vector<TensorDim> dims) override;
   ...
   */
  NNTR_EXPORT int
  setDataset(const DatasetModeType &dt,
   ...
   * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
   */
  NNTR_EXPORT int setDataBuffer(const DatasetModeType &dt,
   ...
   * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
   */
  NNTR_EXPORT int addLayer(std::shared_ptr<ml::train::Layer> layer) override {
   ...
   * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
   */
  NNTR_EXPORT int addLayer(NodeType layer);
   ...
   * @retval #ML_ERROR_INVALID_PARAMETER invalid parameter.
   */
  NNTR_EXPORT int getLayer(const char *name,
   ...
   */
  NNTR_EXPORT std::vector<TensorDim> getInputDimension() override {
    if (!compiled) {
   ...
   */
  NNTR_EXPORT std::vector<TensorDim> getOutputDimension() override {
    if (!compiled) {
      throw std::logic_error("model should be compiled before get dimension");
    }
   ...
   */
  NNTR_EXPORT FlatGraphType getFlatGraph() {
    return model_graph.getLayerNodes();
  }
   ...
   */
  NNTR_EXPORT bool empty() const { return model_graph.empty(); }

  /**
   * @brief get the number of nodes in the model
   * @param[out] number of nodes
   */
  NNTR_EXPORT size_t size() const { return model_graph.size(); }

  /**
   * @brief     get network graph
   * @retval NetowrkGraphType
   */
  NNTR_EXPORT NetworkGraphType getNetworkGraph() { return model_graph; }
   ...
   */
  NNTR_EXPORT GraphType getUnsortedLayers(const std::string &input_layer = "",
  ...
  summarize(std::ostream &out, ml_train_summary_type_e verbosity) override {
   ...
   */
  NNTR_EXPORT virtual void printPreset(std::ostream &out, unsigned int preset);
   ...
   */
  NNTR_EXPORT void enableDynamicTraining(
   ...
   */
  NNTR_EXPORT void disableDynamicFineTuning() {
    dynamic_training_opt.disable();
  }
   ...
   *
   */
  NNTR_EXPORT void addWithReferenceLayers(
   ...
   */
  NNTR_EXPORT void exports(const ml::train::ExportMethods &method,
