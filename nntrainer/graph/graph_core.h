...
public:
   ...
   */
  NNTR_API GraphCore() : node_names(), def_name_count(0) {}

  /**
   * @brief     Destructor of Graph Core Class
   *
   */
  NNTR_API ~GraphCore() = default;

  /**
   * @brief Add the given node into Graph
   * @param[in] node shared_ptr of node
   */
  NNTR_API void addNode(std::shared_ptr<GraphNode> node,
   ...
   */
  NNTR_API unsigned int size() const { return node_list.size(); }

  /**
   * @brief get if the graph is empty
   * @param[out] true if empty, else false
   */
  NNTR_API bool empty() const { return node_list.empty(); }

  /**
   * @brief     Swap function for the class
   */
  NNTR_API friend void swap(GraphCore &lhs, GraphCore &rhs) {
    using std::swap;
   ...
   * @ret GraphNode
   */
  NNTR_API const std::shared_ptr<GraphNode> &getNode(unsigned int ith) const;
   ...
   * @ret GraphNode
   */
  NNTR_API const std::shared_ptr<GraphNode> &
   ...
   * @ret index
   */
  NNTR_API const unsigned int getSortedNodeIdx(const std::string &name) const;
  ...
  NNTR_API inline graph_const_iterator<T> cbegin() const {
    if (Sorted.empty())
  ...
  template <
    typename T = GraphNode,
    std::enable_if_t<std::is_base_of<GraphNode, T>::value, T> * = nullptr>
  NNTR_API inline graph_const_iterator<T> cend() const {
    if (Sorted.empty())
  ...
  template <
    typename T = GraphNode,
    std::enable_if_t<std::is_base_of<GraphNode, T>::value, T> * = nullptr>
  NNTR_API inline graph_const_reverse_iterator<T> crbegin() const {
  ...
  template <
    typename T = GraphNode,
    std::enable_if_t<std::is_base_of<GraphNode, T>::value, T> * = nullptr>
  NNTR_API inline graph_const_reverse_iterator<T> crend() const {
   ...
   */
  NNTR_API GraphCore &copy(GraphCore &from) {
   ...
   */
  NNTR_API void ensureName(GraphNode &node, const std::string &prefix = "",
   ...
   */
  NNTR_API void replaceNode(std::shared_ptr<GraphNode> from,
   ...
   */
  NNTR_API unsigned int getNumInputNodes() const { return input_list.size(); }
   ...
   */
  NNTR_API unsigned int getNumOutputNodes() const { return output_list.size(); }
   ...
   */
  NNTR_API void replaceOutputNode(unsigned int idx,
   ...
   */
  NNTR_API void realizeInputOutputNode();

  /**
   * @brief     Verify if the node exists
   */
  NNTR_API inline bool verifyNode(const std::string &name) {
