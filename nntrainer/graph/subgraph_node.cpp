...
void SubGraphNode::setProperty(const std::vector<std::string> &properties) {
  auto left_properties = loadProperties(properties, *subgraph_props);
}

const std::string SubGraphNode::getName() const noexcept {
  auto &name = std::get<props::SubGraphName>(*subgraph_props);
  return name.empty() ? "default" : name.get();
}

void SubGraphNode::setName(const std::string &name) {
  setProperty({"subgraph_name=" + name});
}

void SubGraphNode::finalize() {

  /** finalize properties */
  subgraph_name = getName();
}

void SubGraphNode::realize(
...
const std::string SubGraphNode::getType() const { return SubGraphNode::type; }

bool SubGraphNode::getTrainable() const {
  return exec_mode == ExecutionMode::TRAIN;
}

const std::vector<std::string> SubGraphNode::getInputConnections() const {
  return cbegin()->getInputConnections();
}

const std::vector<std::string> SubGraphNode::getOutputConnections() const {
  return crbegin()->getOutputConnections();
}

ExecutionOrder SubGraphNode::getExecutionOrder() const { return exec_order; }

void SubGraphNode::setExecutionOrder(ExecutionOrder exec_order_) {
  exec_order = exec_order_;
}

void SubGraphNode::setExecutionOrder() {
...
int SubGraphNode::compile(const std::string &loss_type) {
...
void SubGraphNode::setBatchSize(unsigned int batch_size) {
...
void SubGraphNode::applyGradients(LayerNode *node, int iteration,
...
sharedConstTensors
SubGraphNode::forwarding(bool training,
...
sharedConstTensors SubGraphNode::incremental_forwarding(
...
bool SubGraphNode::backwarding(int iteration,
                               std::function<bool(void *userdata)> stop_cb,
 ...
 */
void SubGraphNode::allocateTensors(ExecutionMode exec_mode_) {
...
std::vector<Var_Grad *>
SubGraphNode::finalizeContext(const std::shared_ptr<LayerNode> &lnode,
...
std::vector<Var_Grad *>
SubGraphNode::refinalizeContext(const std::shared_ptr<LayerNode> &lnode,
                                const std::vector<Var_Grad *> &prev_inputs) {
  const GraphNode &gnode = *lnode.get();
  std::vector<TensorDim> input_dims;
  input_dims.reserve(prev_inputs.size());
...
int SubGraphNode::initialize(ExecutionMode mode,
...
void SubGraphNode::setExternalTensors(const std::vector<Tensor> &data,
...
std::vector<Tensor> SubGraphNode::getOutputTensors() const {
...
void SubGraphNode::LoadTensors(unsigned int order) {
  tensor_manager->LoadTensors(order);
}

bool SubGraphNode::checkLoadComplete(unsigned int order) {
  return tensor_manager->checkLoadComplete(order);
}

bool SubGraphNode::checkUnloadComplete(unsigned int order) {
  return tensor_manager->checkUnloadComplete(order);
}

void SubGraphNode::UnloadTensors(unsigned int order) {
  tensor_manager->UnloadTensors(order);
}

void SubGraphNode::requestOptimizerVariable(
...
void SubGraphNode::forwarding_op(std::shared_ptr<LayerNode> node, bool training,
                                 bool swap_mode) {
  ...
  if (training or (!training and !swap_mode)) {
    ...
    **/
    if (!((tensor_manager->getNumLoadedWeightPoolTensors() + 1) / 2 <
          lookahead + 1)) {
      tensor_manager->checkUnloadComplete(f - 1);
    }
    tensor_manager->LoadTensors(
      f, lookahead - (tensor_manager->getNumLoadedWeightPoolTensors() + 1) / 2);

    tensor_manager->checkLoadComplete(f);
    node->forwarding(training);
    tensor_manager->UnloadTensors(f);
  }
}

void SubGraphNode::incremental_forwarding_op(std::shared_ptr<LayerNode> node,
...
bool SubGraphNode::backwarding_op(std::shared_ptr<LayerNode> node,
                                  int iteration,
                                  std::function<bool(void *userData)> stop_cb,
                                  void *user_data, bool is_grad_opt_mode,
                                  std::shared_ptr<OptimizerWrapped> opt) {
...
void SubGraphNode::lazy_apply_grad_op(Weight &w, int iteration,
                                      std::shared_ptr<OptimizerWrapped> opt) {
  w.calcRegularizationGradient();
  w.calcWeightDecayGradient();
  RunOptimizerContext opt_context(&w, iteration,
...
void SubGraphNode::flushCache() { tensor_manager->flushCache(); }

void SubGraphNode::flushCacheExcept(unsigned int order) {
  tensor_manager->flushCacheExcept(order);
}

int SubGraphNode::addLossLayer(const std::string &loss_type_) {
  for (unsigned int i = 0; i < subgraph.getNumOutputNodes(); ++i) {
...
void SubGraphNode::setOutputConnections() {
...
int SubGraphNode::checkCompiledGraph() {
...
void SubGraphNode::markNodesForBackwarding() {
...
LayerNode *SubGraphNode::computeBackwardEnd() {
...
std::vector<TensorDim> SubGraphNode::getInputDimension() const {
  NNTR_THROW_IF(input_dims.empty(), std::invalid_argument)
    << "[SubGraphNode] the graph has no node identified as input!";
  return input_dims;
}

unsigned int SubGraphNode::getBatchSize() const { return batch_size; }

std::vector<TensorDim> SubGraphNode::getOutputDimension() const {
  NNTR_THROW_IF(label_dims.empty(), std::invalid_argument)
    << "[SubGraphNode] the graph has no node identified as output!";
...
SubGraphNode::getUnsortedLayers(const std::string &input_layer,
...
std::vector<std::shared_ptr<LayerNode>> SubGraphNode::getLayerNodes() const {
  return std::vector<std::shared_ptr<LayerNode>>(cbegin(), cend());
}

void SubGraphNode::addLayer(std::shared_ptr<LayerNode> layer) {
  if (compiled)
...
InPlaceType
SubGraphNode::canExecuteInPlace(const std::shared_ptr<LayerNode> &lnode) {
...
void SubGraphNode::inPlaceOptimize() {
  if (optimize_memory) {
    for (unsigned int idx = 0; idx < subgraph.size(); ++idx) {
      auto const &lnode = getSortedLayerNode(idx);
...
SubGraphNode::getLayerExecutionOrders(const std::shared_ptr<LayerNode> &lnode) {
...
void SubGraphNode::setInputsLabels(const std::vector<Tensor> &inputs,
                                   const std::vector<Tensor> &labels) {
  setInputs(inputs);
  setLabels(labels);
}

void SubGraphNode::setInputsLabels(sharedConstTensors &inputs,
                                   sharedConstTensors &labels) {
  setInputs(inputs);
  setLabels(labels);
}

void SubGraphNode::setInputs(const std::vector<Tensor> &inputs) {
...
void SubGraphNode::setInputs(sharedConstTensors &inputs) {
...
void SubGraphNode::setLabels(const std::vector<Tensor> &labels) {
...
void SubGraphNode::setLabels(sharedConstTensors &labels) {
...
void SubGraphNode::resetLossScale(float scale) {
  loss_scale = scale;
  for (auto iter = cbegin(); iter != cend(); iter++) {
    auto &ln = *iter;
...
void SubGraphNode::read(std::ifstream &file, bool opt_var,
...
void SubGraphNode::save(std::ofstream &file, bool opt_var,
...
float SubGraphNode::getLoss() const {
  auto loss = 0.0f;
...
void SubGraphNode::clearOptVar() {
  for (auto layer_iter = cbegin(); layer_iter != cend(); ++layer_iter) {
    (*layer_iter)->clearOptVar();
  }
}

void SubGraphNode::printPreset(std::ostream &out, PrintPreset preset) {
  for (auto layer_iter = cbegin(); layer_iter != cend(); ++layer_iter) {
...
createSubGraphNode(const ml::train::SubGraphType &type,
  ...
  ///@todo AppContext registration
  // auto &ac = nntrainer::AppContext::Global();
  // return createSubGraphNode(ac.createObject<SubGraphNode>(type), properties);

  ///@todo Apply type info
  std::unique_ptr<SubGraphNode> ptr = std::make_unique<SubGraphNode>();
  ptr->setProperty(properties);
  return ptr;
}
...
SubGraphType createSubGraph(const std::vector<std::string> &properties) {
  const auto &sg_node = SGNODE(std::make_shared<SubGraphNode>());
