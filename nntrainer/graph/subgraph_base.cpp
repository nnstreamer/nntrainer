...
void SubGraphBase::setBatchSize(unsigned int batch_size) {
 ...
 */
void SubGraphBase::allocateTensors(ExecutionMode exec_mode_) {
...
std::vector<Var_Grad *>
SubGraphBase::finalizeContext(const std::shared_ptr<LayerNode> &lnode,
...
std::vector<Var_Grad *>
SubGraphBase::refinalizeContext(const std::shared_ptr<LayerNode> &lnode,
                                const std::vector<Var_Grad *> &prev_inputs) {
  const GraphNode &gnode = *lnode.get();
  std::vector<TensorDim> input_dims;
  input_dims.reserve(prev_inputs.size());
...
int SubGraphBase::initialize(ExecutionMode mode,
...
void SubGraphBase::flushCache() { tensor_manager->flushCache(); }

void SubGraphBase::flushCacheExcept(unsigned int order) {
