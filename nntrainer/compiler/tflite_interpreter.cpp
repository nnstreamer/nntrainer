...
public:
  ...
  void addDataWhenNotFound(KeyType key, DataType data) {
    ...
    if (search == data2index.end()) {
      data2index[key] = index2data.size();
...
private:
  std::unordered_map<KeyType, unsigned int>
    data2index;                     /**< data -> index map */
...
public:
  ...
  BidirectionalIndexMap<KT, DT> &getIndexMap() {
    return std::get<BidirectionalIndexMap<KT, DT>>(maps);
  }

  template <typename KT, typename DT = KT>
  const BidirectionalIndexMap<KT, DT> &getIndexMap() const {
    return std::get<BidirectionalIndexMap<KT, DT>>(maps);
  }
