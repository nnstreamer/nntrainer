...
GraphStatus llamadequantizeImpl(TensorType1 &out_0, const TensorType1 &in_0,
 ...
 * DEF_PACKAGE_OP((llamadequantizeImpl<Tensor, Tensor, Tensor>),
 * "LLaMADequantize")
 */
 ...
 * DEF_PACKAGE_OP_AND_COST_AND_FLAGS((llamadequantizeImpl<PlainFloatTensor,
 * PlainFloatTensor, PlainFloatTensor>), "LLaMADequantize", SNAIL)
 */
 ...
 * e.g.
 * DEF_PACKAGE_OP_AND_COST_F_AND_FLAGS((llamadequantizeImpl<PlainFloatTensor,
 * PlainFloatTensor, PlainFloatTensor>), "LLaMADequantize",
 * llamadequantizeCostFunc, Flags::RESOURCE_HVX)
 */
...
/* execute functions for ops */
int32_t qhmath_hvx_dequantize_ahf(int8_t *restrict input,
                                  int8_t *restrict output, uint32_t size,
                                  float scale) {
  ...
  for (int32_t i = vectors_in_rounddown - 1; i > 0; i -= BLOCK_SIZE) {
    ...
    for (int32_t j = 0; j < block; ++j) {
      ...
      *optr++ = Q6_Vhf_equals_Vqf16(
        Q6_Vqf16_vmpy_VhfVhf(Q6_Vhf_equals_Vh(sout1), scale_vec));
      *optr++ = Q6_Vhf_equals_Vqf16(
        Q6_Vqf16_vmpy_VhfVhf(Q6_Vhf_equals_Vh(sout2), scale_vec));
...
int32_t qhmath_hvx_dequantize_af(int8_t *restrict input,
                                 int8_t *restrict output, uint32_t size,
                                 float scale) {
  if ((input == NULL) || (output == NULL) || (size == 0)) {
    return -1;
