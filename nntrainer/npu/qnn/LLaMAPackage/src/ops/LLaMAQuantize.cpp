...
GraphStatus llamaquantizeImpl(TensorType1 &out_0, const TensorType1 &in_0,
 ...
 * DEF_PACKAGE_OP_AND_COST_AND_FLAGS((llamaquantizeImpl<PlainFloatTensor,
 * PlainFloatTensor, PlainFloatTensor>), "LLaMAQuantize", SNAIL)
 */
 ...
 * e.g. DEF_PACKAGE_OP_AND_COST_F_AND_FLAGS((llamaquantizeImpl<PlainFloatTensor,
 * PlainFloatTensor, PlainFloatTensor>), "LLaMAQuantize", llamaquantizeCostFunc,
 * Flags::RESOURCE_HVX)
 */
...
int32_t qhmath_hvx_quantize_ahf(__fp16 *restrict input, __fp16 *restrict output,
                                uint32_t size, float low_level,
...
int32_t qhmath_hvx_quantize_ahf_int8(__fp16 *restrict input,
                                     __fp16 *restrict output, uint32_t size,
                                     float low_level, float high_level,
                                     float scale) {
  if ((input == NULL) || (output == NULL) || (size == 0)) {
    return -1;
  }
...
int32_t qhmath_hvx_quantize_af(float *restrict input, float *restrict output,
                               uint32_t size, float low_level, float high_level,
                               float scale) {
  if ((input == NULL) || (output == NULL) || (size == 0)) {
    return -1;
  }
...
int32_t qhmath_hvx_quantize_af_out_int8(float *restrict input,
...

{
  ...
  if (dtype == DType::Float16 && out_0.get_dtype() == DType::QInt8) {
    ...
    qhmath_hvx_quantize_ahf_int8(in_ptr, out_ptr, size, -128.0f, 127.0f,
                                 scale_);
  }
  ...
  if (dtype == DType::Float32 && out_0.get_dtype() == DType::QInt8) {
    ...
    auto in_ptr = (float *)in_0.raw_data_const();
    auto out_ptr = (float *)out_0.raw_data();
    qhmath_hvx_quantize_af_out_int8(in_ptr, out_ptr, size, -128.0f, 127.0f,
                                    scale_);
  }
