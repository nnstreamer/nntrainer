 ...
 * @todo   Input axis can't be negative. Have to add support for negative axis
 * input. Negative axis handling in unsqueeze layer is implemented
 *
 */
...
void UnsqueezeLayer::finalize(InitLayerContext &context) {
  ...
  NNTR_THROW_IF((input_axis > (input_ndim) || input_axis < (-1 - input_ndim)),
                std::invalid_argument)
    << "Unsqueeze layer must have axis in range [" << (-1 - input_ndim) << ","
    << (input_ndim) << "] for input_ndims " << input_ndim << " ."
    << " Your axis is " << input_axis;

  if (axis < 0) {
    axis.set(input_axis + input_ndim + 1); // negative axis handling
  }

  axis.set((in_dim.getNumDim() + input_axis - input_ndim -
            1)); // setting axis relative to 4D tensor.
