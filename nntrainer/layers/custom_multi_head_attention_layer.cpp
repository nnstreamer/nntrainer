 ...
 * @brief  This is CustomMultiHeadAttention Layer Class simulating the same from
 * FELICE repo
 *
 */
...
CustomMultiHeadAttentionLayer::CustomMultiHeadAttentionLayer() :
...
void CustomMultiHeadAttentionLayer::finalize(
  nntrainer::InitLayerContext &context) {
  ml_logi("MHA cl: finalize called");
...
void CustomMultiHeadAttentionLayer::forwarding(
  nntrainer::RunLayerContext &context, bool training) {}

void CustomMultiHeadAttentionLayer::one_batch_incremental_forwarding(
  ...
  //////////////////////////////////////////////////////////
  // to do: use blas_kernel dot for GPU
  ...
  ////////////////////////////////////////////////////////////////
  if (!disable_bias) {
    // to do: use blas_kernel add for GPU (verify with Yash)
  ...
  projected_query_step.dotBatched(projected_key_step, attention_weight_step,
                                  false, true);
  //////////// test OpenCL impl //////////////////////
  // mha_dotBatchProcess(projected_query_step, projected_key_step,
  //                     attention_weight_step, false, true, context);
  //////////////////////////////////////////////////////
  attention_weight_step.multiply_i(1 / sqrt((float)projected_query_dim_prop));
  //////////// test OpenCL impl //////////////////////
  // mha_multiplyProcess(attention_weight_step, 1 /
  // sqrt((float)projected_query_dim_prop));
  //////////////////////////////////////////////////////
  if (!from) {
...
void CustomMultiHeadAttentionLayer::incremental_forwarding(
  ...
  ml_logi("from: %d, to: %d, max_timestep: %d", _from, _to, max_timestep);
  ...
  //     std::get<nntrainer::props::ReturnAttentionWeight>(
  //                                 multi_head_attention_props)
  //                                 .get();
  //   const bool average_attention_weight =
  //     std::get<nntrainer::props::AverageAttentionWeight>(
  ...
  //     provide_attention_mask ? context.getInput(INOUT_INDEX::MASK) :
  //     empty_tensor;
  ...
  ////////////////////testing////////////////
  ml_logi("projected key dim: %d", projected_query_dim_prop);
  ml_logi("query - b: %d, c: %d, h: %d, w: %d", query_dim.batch(),
          query_dim.channel(), query_dim.height(), query_dim.width());
  ml_logi("key - b: %d, c: %d, h: %d, w: %d", key_dim.batch(),
          key_dim.channel(), key_dim.height(), key_dim.width());
  ml_logi("value - b: %d, c: %d, h: %d, w: %d", value_dim.batch(),
          value_dim.channel(), value_dim.height(), value_dim.width());
  //////////////////////////////////////////
  ...
  for (unsigned int batch = 0; batch < b_size; ++batch) {
    one_batch_incremental_forwarding(
      ...
      attention_output_dim, attention_output_step_dim);
  }
...
void CustomMultiHeadAttentionLayer::calcCommonDerivative(
  nntrainer::RunLayerContext &context) {}

void CustomMultiHeadAttentionLayer::calcDerivative(
  nntrainer::RunLayerContext &context) {}

void CustomMultiHeadAttentionLayer::calcGradient(
  nntrainer::RunLayerContext &context) {}

void CustomMultiHeadAttentionLayer::setProperty(
  const std::vector<std::string> &values) {
  auto remain_props = loadProperties(values, multi_head_attention_props);
  LayerImpl::setProperty(remain_props);
}

void CustomMultiHeadAttentionLayer::setBatch(
...
void CustomMultiHeadAttentionLayer::exportTo(
