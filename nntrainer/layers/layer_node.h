...
public:
   ...
   *
   */
  NNTR_API LayerNode(std::unique_ptr<nntrainer::Layer> &&l);
  ...
  NNTR_API std::string getProperty(const std::string &key) override;
   ...
   */
  NNTR_API const std::string getName() const override;
   ...
   */
  NNTR_API void setName(const std::string &name) override {
   ...
   */
  NNTR_API void setDataType(const TensorDim::DataType w_type,
   ...
   */
  NNTR_API const TensorDim::DataType getWeightDataType() const {
    return data_type[0];
  }
   ...
   */
  NNTR_API const TensorDim::DataType getActivationDataType() const {
    return data_type[1];
  }
   ...
   */
  NNTR_API const unsigned getInputConnectionIndex(unsigned nth) const;
   ...
   */
  NNTR_API const std::string &getInputConnectionName(unsigned nth) const;
   ...
   * @throws if nth is out of range of getNumInputConnection()
   */
  NNTR_API void setInputConnectionIndex(unsigned nth, unsigned index);
   ...
   */
  NNTR_API void setInputConnectionName(unsigned nth, const std::string &name);
   ...
   */
  NNTR_API const Connection *getOutputConnection(unsigned nth) const;
   ...
   */
  NNTR_API void setOutputConnection(unsigned nth, const std::string &name,
                                    unsigned index);
   ...
   */
  NNTR_API const std::vector<std::string> getInputConnections() const override {
   ...
   */
  NNTR_API const std::vector<std::string>
   ...
   */
  NNTR_API ExecutionOrder getExecutionOrder() const override {
    return exec_order;
  }
   ...
   */
  NNTR_API void setExecutionOrder(ExecutionOrder exec_order_) override {
  ...
  NNTR_API void initialize() override { layer->initialize(*run_context); }
   ...
   */
  NNTR_API void forwarding(bool training = true);
   ...
   * can be access from the inputs/outputs tensors themselves.
   */
  NNTR_API void incremental_forwarding(unsigned int from, unsigned int to,
                                       bool training = true);
   ...
   */
  NNTR_API bool supportInPlace() const;
   ...
   */
  NNTR_API InPlaceType getInPlaceType() const { return inplace_type; }
   ...
   */
  NNTR_API InPlaceDirection getInPlaceDirection() const;
   ...
   */
  NNTR_API bool requireLabel() const;
   ...
   */
  NNTR_API bool supportBackwarding() const {
   ...
   */
  NNTR_API void setOutputLayers(const std::vector<std::string> &layers);
   ...
   */
  NNTR_API bool hasInputShapeProperty() const;
   ...
   */
  NNTR_API const std::vector<TensorDim> getOutputDimensions() const;
  /**
   ...
   */
  NNTR_API Weight getWeightWrapper(unsigned int idx) {
    NNTR_THROW_IF(!run_context, std::runtime_error)
      << __func__ << " layer needs to be finalized first!";
   ...
   * @return Tensor& Reference to the weight tensor
   */
  NNTR_API Tensor &getWeight(unsigned int idx) {
    NNTR_THROW_IF(!run_context, std::runtime_error)
      << __func__ << " layer needs to be finalized first!";
   ...
   */
  NNTR_API Tensor &getWeightGrad(unsigned int idx) {
    NNTR_THROW_IF(!run_context, std::runtime_error)
      << __func__ << " layer needs to be finalized first!";
   ...
   */
  NNTR_API const std::string &getWeightName(unsigned int idx) override {
    NNTR_THROW_IF(!run_context, std::runtime_error)
      << __func__ << " layer needs to be finalized first!";
   ...
   */
  NNTR_API const std::vector<float *> getWeights() override {
    NNTR_THROW_IF(!run_context, std::runtime_error)
      << __func__ << " layer needs to be finalized first!";
   ...
   * @note      layer needs to be finalized before called.
   */
  NNTR_API void getWeights(std::vector<float *> &weights,
   ...
   * @note      layer needs to be finalized before called.
   */
  NNTR_API void getFP16Weights(std::vector<_FP16 *> &weights,
                               std::vector<TensorDim> &weight_dim) override {
    NNTR_THROW_IF(!run_context, std::runtime_error)
      << __func__ << " layer needs to be finalized first!";
   ...
   * @note      layer needs to be finalized before called.
   */
  NNTR_API void setWeights(const std::vector<float *> weights) override;
   ...
   * @return Tensor& Reference to the output grad tensor
   */
  NNTR_API const Tensor &getOutputGradUnsafe(unsigned int idx) const {
   ...
   *
   */
  NNTR_API void clearOptVar();

  /**
   * @brief     get loss for the layer
   * @return    loss of the layer
   */
  NNTR_API float getLoss() const;
   ...
   */
  NNTR_API friend std::ostream &operator<<(std::ostream &out,
                                           const LayerNode &l);
   ...
   */
  NNTR_API RunLayerContext &getRunContext() {
    NNTR_THROW_IF(!run_context, std::runtime_error)
   ...
   * @retval  run layer context
   */
  NNTR_API const RunLayerContext &getRunContext() const {
    NNTR_THROW_IF(!run_context, std::runtime_error)
      << __func__ << " layer needs to be configured first!";
    return *run_context;
  }
   ...
   */
  NNTR_API bool isFinalized() const {
    if (!run_context)
      return false;
   ...
   */
  NNTR_API void printPreset(std::ostream &out,
   ...
   */
  NNTR_API void remapIdentifiers(std::function<void(std::string &)> remap_fn);
   ...
   */
  NNTR_API std::unique_ptr<LayerNode> cloneConfiguration();
   ...
   */
  NNTR_API void needsCalcDerivative(bool nb) {
   ...
   */
  NNTR_API void reStoreData(bool nb) {
   ...
   * @param nb true if the layer needs to do backwarding, else false
   */
  NNTR_API void needsCalcGradient(bool nb) { needs_calc_gradient = nb; }
   ...
   */
  NNTR_API bool needsCalcDerivative() { return needs_calc_derivative; }
   ...
   * @param nb true if the layer needs to do backwarding, else false
   */
  NNTR_API bool needsCalcGradient() { return needs_calc_gradient; }
   ...
   */
  NNTR_API bool reStoreData() { return needs_restore_data; }

  NNTR_API std::string getComputeEngineType() {
...
private:
   ...
   */
  NNTR_API const std::vector<std::string> getInputLayers() const;
   ...
   * @return const std::vector<std::string>
   */
  NNTR_API const std::vector<std::string> getOutputLayers() const;
...
properties in the context/graph unless intended. */
   ...
   * is distributed.
   */
  NNTR_API const nntrainer::Layer *getLayer() const;
   ...
   */
  NNTR_API void print(std::ostream &out, unsigned int flags = 0);
};
 ...
 */
NNTR_API std::unique_ptr<LayerNode>
