...
void AttentionLayer::incremental_forwarding(RunLayerContext &context,
  ...
  Tensor output_step =
    from ? output.getSharedDataTensor(output_step_dim,
                                      from * output_dim.width(), true)
         : Tensor(output_step_dim);
  ...
  if (!from) {
    for (unsigned int i = 0; i < output_step_dim.height(); ++i) {
      TensorDim output_seq_step_dim = {output_step_dim.batch(),
                                       output_step_dim.channel(), 1,
                                       output_step_dim.width()};
      Tensor output_step_src = output_step.getSharedDataTensor(
        output_seq_step_dim, i * output_step_dim.width(), true);
      Tensor output_step_dst = output.getSharedDataTensor(
        output_seq_step_dim, i * output.width(), true);
      output_step_dst.copyData(output_step_src);
    }
  }
}
