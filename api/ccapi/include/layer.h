...
enum LayerType {
  ...
  LAYER_CONVTRANSPOSE2D = ML_TRAIN_LAYER_TYPE_CONVTRANSPOSE2D, /**< Convolution 2D Layer type */
...
/**
 * @brief Helper function to create transposed convolution 2d layer
 */
inline std::unique_ptr<Layer>
ConvolutionTranspose2D(const std::vector<std::string> &properties = {}) {
  return createLayer(LayerType::LAYER_CONVTRANSPOSE2D, properties);
}
