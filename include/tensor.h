...
class Tensor {
 public:
   ...
   */
  Tensor(std::vector<std::vector<double>> const &data);

  /**
   * @brief     Constructor of Tensor
   * @param[in] data data for the Tensor
   */
  Tensor(std::vector<std::vector<std::vector<double>>> const &data);
   ...
   */  
  Tensor multiply(double const &value);
   ...
   * @retval    Calculated Tensor
   */  
  Tensor divide(double const &value);
   ...
   * @retval    Tensor
   */  
  Tensor applyFunction(double (*function)(double)) const;
